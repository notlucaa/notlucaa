...
const runVortex = () => {
    ...
    if (!vortexContainer || vortexStarted) return;

    if (typeof THREE === 'undefined') {
        // Stop if not on a page that needs it or if Three.js is missing
        if (!window._vortexRetries) window._vortexRetries = 0;
        if (window._vortexRetries < 5) {
            window._vortexRetries++;
            setTimeout(runVortex, 200);
        }
        return;
    }
